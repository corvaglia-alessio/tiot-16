...
void setup()
{
    ...
    mqtt.subscribe("/tiot/16/yun/lampadina", regola_lampadina);
}
...
void regola_lampadina(const String& topic, const String& subtopic, const String& message)
{
  ...
  if(e){
    ...
    if(doc_rcv["e"][0]["n"]=="lampadina"){
      int statuss = (float) doc_rcv["e"][0]["v"];
      if(statuss == 0 || statuss==1){
        digitalWrite(LAMPADINA_PIN, statuss);
        Serial.print("Lampadina regolata");
        Serial.println(statuss);
      }
    else{
        Serial.println("Error: status for lampadina not valid");
      }
    }
  } 
}
